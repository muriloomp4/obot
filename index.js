...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'BOT DO MURILOO ESTÁ ON  🟢')
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que goste do grupo, leia as regras e seja feliz❤️`
				...
				teks = `Tchau @${num.split('@')[0]} até algum dia soldado 😔`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬👉🏻❗👈🏻 ❭AGUARDE, CASO O BOT NÃO RESPONDA, TENTE NOVAMENTE',
					success: '️❬ ✔ ❭ Sucesso 🖤',
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD MURILOO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Este é um recurso especial para o Muriloo ❌',
					ownerB: '[❗] Este é um recurso especial para o Muriloo❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de MURILOO. Registre-se para fazer amizade com o bot Muriloo por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Muriloo|17\n\n──「 MURILOO BOT 」──`,
			...
			const ownerNumber = ["555597294998@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"555597294998@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["555597294998@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["555597294998@s.whatsapp.net"]//ubah nomor lo
			const premium = ["555597294998@s.whatsapp.net","558296156440@s.whatsapp.net","558197995414@s.whatsapp.net","555596350424@s.whatsapp.net","555496222190@s.whatsapp.net","553388658467@s.whatsapp.net","5521965204100@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau 😔🤙")
		}, 0)
	}
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("você, seu cagão")
	}
	
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("como eu seria corno, se eu não namoro, e não jogo fogo gratuito?")
	}
	
		if (messagesC.includes("tmnc")){
			client.updatePresence(from, Presence.composing)
			reply("vai você doente")
	}
	
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("fica xiu aí, filho kkkkkkkkkkkkkkkk")
	}
	
		if (messagesC.includes("cadebot")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui cara 😔")
	}
	
		if (messagesC.includes("oibot")){
			client.updatePresence(from, Presence.composing)
			reply("oooii tô on desgraça kkkk")
	}
...
if (messagesC.includes("freefire")){
			client.updatePresence(from, Presence.composing)
			reply("se você joga firo fare exclua meu contato imediatamente")

}

if (messagesC.includes("V6")){
			client.updatePresence(from, Presence.composing)
			reply("https://chat.whatsapp.com/HYIEOoqsGUwF0XaWGwVfPp")

} 


			if (messagesC.includes("dono")){
			client.updatePresence(from, Presence.composing)
			reply("wa.me/555597294998 aqui meu dono lindo")
	}
...
if (messagesC.includes("bah")){
	...
	if (messagesC.includes("cai")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/cai.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	...
	if (messagesC.includes("papão")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/papao.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	...
	if (messagesC.includes("menu")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/aud.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("heathens")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/heathens.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}
	
	if (messagesC.includes("funk2")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/ak.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("nowhere")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/nowhere.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("lofi")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/lofi.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("funk")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/funk.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("temaki")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/temaki.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
if (messagesC.includes("hope")){
	...
	if (messagesC.includes("instagram")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/insta.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	if (messagesC.includes("brinde")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/brinde.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
if (messagesC.includes("groupies")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/groupies.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("100")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/100.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("manha")){
			client.updatePresence(from, Presence.composing)
	...
	if (messagesC.includes("numb")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/numb.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}
	
	if (messagesC.includes("surreal")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/surreal.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	} 
	
	if (messagesC.includes("etpm")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/etpm.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}
	
	if (messagesC.includes("daddy")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/ngc.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	} 
	
	if (messagesC.includes("triumph")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/xxx.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}
	
	if (messagesC.includes("saque")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/saque.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}
	

if (messagesC.includes("roxo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/roxo.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("saudade")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/saudade.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})

	}

if (messagesC.includes("swing")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/swing.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
if (messagesC.includes("opa")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/opa.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	...
	if (messagesC.includes("tokyo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tokyo.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	...
	if (messagesC.includes("barões")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/baroes.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			if (!isGroup && isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'do menino(a) ', color(sender.split('@')[0]), 'args :', color(args.length))
			if (!isGroup && !isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Message'), 'do menino(a) ', color(sender.split('@')[0]), 'args :', color(args.length))
			if (isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'do menino(a) ', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			if (!isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Message'), 'do menino(a) ', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			switch(command) {
				case 'help':
				case 'menu':
client.sendMessage(from, help(prefix) , text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "@muriloo.mp4", 'jpegThumbnail': fs.readFileSync('muriloo/bot/imgbot.webp')}}}})					
break
				case 'help1':
				case 'menu1':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS MURILOO BOT 👑* 」\n\`\`\``
					no = 0
					...
					break
				case 'desligar':
				if (!isOwner) return reply('me perdoe, amigo, mas este comando só pode ser usado pelo meu dono 😔')
					...
					break
					case 'dono1':                     
					data = await fetchJson(`https://pastebin.com/raw/yQsdJdSr`, {method: 'get'})
                     anuk = await getBuffer(data.img)
                     dark = `âž³ NOME: ${data.nome}\nâž³ NUMERO: ${data.numero}\nâž³ SEXO: ${data.sexo}\nâž³ NICK: ${data.nick}\nâž³ YT: ${data.yt}\nâž³ INSTA:  ${data.insta}`                     
...
// recursos anime, random @Muriloo domina vida
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "muriloo", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'contato do meu dono aqui ',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 's':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas *.f* ou marque uma imagem que já foi enviada, se for fazer com videos, o vídeo/gif deve ter menos de 7 segundos`)
					}
					break
				case 'tts':
				...
				if (!isOwner) return reply(mess.only.ownerB)
                 ...
                 case '3dt':
                   txt = body.slice(6)
                  post = await fetchJson(`https://api-exteam.herokuapp.com/api/photooxy?tema=harry-poter&text=${txt}`)
                  send = await getBuffer(post.result)
                  client.sendMessage(from, send, image, {quoted: mek})
                  break
				case '3d':
				if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply(mess.blank)
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Muriloo ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT MURILOO ✨* 」\n*│+ wa.me/555597294998*╰──────*「 *MURILOO* 」*────*\n\n*_SE QUER SER ADMIN DO MURILOO BOT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
                ...
                break
                 ...
                 case 'calculadora':
				    ...
				    break 
				case 'pornomia':
				    if (!isNsfw)return reply(mess.only.group)
					if (!isNsfw)return reply(mess.only.admin)
				...
				case 'belle1':
					...
					break
					case 'linallis':                 
				 data = fs.readFileSync('./src/linallis.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*Linallis :V*')
				break
				
				case 'orochinho':                 
				 data = fs.readFileSync('./src/orochinho.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*orochi :V*')
				break
				
				case 'breely':                 
				 data = fs.readFileSync('./src/breely.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*breely :V*')
				break
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do MURILOO:*\n\n https://youtube.com/channel/UCdG5k77qTO23axwPEylD69A'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.pinimg.com/736x/6c/e0/74/6ce074d297b9400f0bbc78863a101cdb.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '➥CRIADOR: MURILOO\n ➥YOUTUBE: https://youtube.com/channel/UCdG5k77qTO23axwPEylD69A\n ➥WHATS:  wa.me/+555597294998\n ➥INSTA: @muriloo.mp4\n\n\nEspero que tenham gostado do bot 🙄❤️ '})
					break
					case 'lolinazista':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://pbs.twimg.com/media/D-frjGpXoAA1ltI.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '🙄➥BELO ARGUMENTO, PORÉM LOLI NAZISTA\n 🙄➥SEGUE MEU DONO:https://youtube.com/channel/UCdG5k77qTO23axwPEylD69A\n 🙄➥CHAMA MEU DONO NO ZAPE ZAPE:  wa.me/+555597294998\n 🙄➥SEGUE MEU DONO NO INSTA: @muriloo.mp4\n\n\nPARE DE VER LOLIS, AMIGO, FBI VAI TE PEGAR'})
					break
					case 'alina':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.pinimg.com/originals/2a/d2/89/2ad289f8bc22be7c29fc8d5b3f5fb816.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'PARE AMIGO ISSO NÃO É VIDA😔🤙🏻 '})
					break
					
					case 'jordan':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://swcstore.com.br/wp-content/uploads/2020/11/WhatsApp-Image-2020-11-25-at-18.28.08-1-e1606339949637.jpeg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'É DEMAIS QUERER UM DESSES? 😔🤙🏻 '})
					break

                    case 'bv':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://static.todamateria.com.br/upload/be/mv/bem-vindo-benvindo-og.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'BEM VINDO AO GRUPO, AMIGÃO😁'})
					break
        ...
        case 'promover':
				...
				case 'banir':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('MARCA O RETARDADO QUE VOCÊ QUER BANIR!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'OK, CHEFE, UM RETARDADO FOI RETIRADO DO GRUPO KKKKK:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
               ...
               case 'timg':
				...
				case 'clonar':
					if (!isOwner) return reply('me perdoe, amigo, mas este comando só pode ser usado pelo meu dono 😔')
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('não use este comando ou levará block do bot\n\n*EXEMPLO:* clone @')
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'Comando inexistente ou mensagem de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
